...
function Home() {
  ...
  // submit handel
  const clickHandel = () => {
    axios.post("https://my-task-server-six.vercel.app/task", { task: change }).then((res) => {
  ...
  // get data
  useEffect(() => {
    axios.get("https://my-task-server-six.vercel.app/task").then((res) => setTask(res.data));
  }, []);
  // isOk handle
  const isOkHandel = (id, isOk) => {
    ...
    if (!isOk) {
      axios
        .post("https://my-task-server-six.vercel.app/task/checked", { id })
  ...
  const deletHandel = (id) => {
    axios
      .delete(`https://my-task-server-six.vercel.app/task/delete/${id}`)
      .then(() => location.reload());
  };
